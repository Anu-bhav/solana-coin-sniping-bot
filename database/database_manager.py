...
class DatabaseManager:
    """Manages database connections and operations using asyncpg"""
    ...
    async def add_detection(
        ...
        """Insert new detected token into detections table"""
    ...
    async def update_detection_status(
        ...
        """Update detection status and filter failure reason"""
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute(
                    """
    ...
    async def add_position(
        ...
        """UPSERT position with conflict resolution on token_mint"""
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute(
                    """
    ...
    async def get_active_position(self, token_mint: str) -> Optional[Dict]:
        """Retrieve active position by token mint"""
        async with self.pool.acquire() as conn:
    ...
    async def check_if_token_processed(self, token_mint: str) -> bool:
        """Check if token exists in either detections or positions with terminal status"""
        async with self.pool.acquire() as conn:
